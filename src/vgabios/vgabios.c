...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
// Start of main Assembly section:
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
static Bit8u vgabios_start[]  = { 0x55, 0xaa };    // BIOS signature, required for BIOS extensions
static Bit8u vgabios_length[] = { 0x40 };          // BIOS signature, required for BIOS extensions
static Bit8u jump_far_ptr[]   = { 0xEA, 0x00, 0x10, 0x00, 0xC0 }; // This is jmp far ptr vgabios_init_func  
static char vgabios_name[]    = "Zet VGA bios";
static char IBM_String[]      = "IBM";             // Not sure how to force this string to    org 0x001e   ??

#ifndef BIOS_VERS
static char vgabios_version[] = "git";
#else
static char vgabios_version[] = "Special Build ";
#endif

static char vgabios_date[]      = "May 20, 2010\n\r";
static char vgabios_copyright[] = "(C) 2003 the LGPL VGABios developers Team\n\r";
static char vgabios_license[]   = "This VGA/VBE Bios is released under the GNU LGPL\n\r\n\r";
static char vgabios_website[]   = "Please visit :\n\r"
                                  "zet.aluzina.org\n\r"
                                  "  bochs.sourceforge.net\n\r"
                                  "  www.nongnu.org/vgabios\n\r\n\r";
...
static void vgabios()
{
    __asm {
            ...
            jmp vgabios_init_func  // force a far jump
    }
}
...
__asm {
            mov     dx, 0x03C2  // switch to color mode and enable CPU access 480 lines
            mov     al, 0xC3
            out     dx, al
...
#if defined(USE_BX_INFO) || defined(DEBUG)
msg_vga_init:
            db      'VGABios $Id: vgabios.c,v 1.66 2006/07/10 07:47:51 vruppert Exp $'
            db      0x0d, 0x0a, 0x00
#endif
    }
}
...
{
    __asm {                  // Get length of string
            mov     si, word ptr ascii_string;
...
Bit8u flag;Bit8u page;Bit8u attr;Bit16u count;Bit8u row;Bit8u col;Bit16u seg;Bit16u offset;
{
    ...
    biosfn_get_cursor_pos(page,&dummy,&oldcurs);   // Read curs info for the page
